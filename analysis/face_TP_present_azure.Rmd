...
d0 <- read.csv('data/face_TP_present_YW.csv')
str(d0)
```
...
d0_emo_age <- d0 %>%
  ...
  select(faceId, age, gender, label) %>%
  drop_na() %>%
...
## load data
```{r}
d <- read.csv('data/Headcam_emotion_annotations_all.csv', fill = TRUE, header = FALSE)
head(d)
View(d)
```
## clean data
...
ggsave(here("human.pdf"), width = 8, height = 12)
```
