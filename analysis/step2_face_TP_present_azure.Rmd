...
## Merge human and azure annotations
...
merged <- d.tidy %>%
  ...
  left_join(d0_emo_age, by='filename') %>%
  filter(!is.na(filename))
```

```{r}
recall_by_emotion <- merged %>%
  group_by(human_label) %>%
  filter(!is.na(human_label)) %>%
  summarize(percent_agree = mean(human_label == label), num_cases = length(human_label))
```
...
ggplot(recall_by_emotion, aes(x = human_label, y = percent_agree, size = num_cases, color = human_label)) +
  theme_few() +
  geom_point()
```
